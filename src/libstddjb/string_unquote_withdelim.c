...
int string_unquote_withdelim (char *d, unsigned int *w, char const *s, unsigned int len, unsigned int *r, char const *delim, unsigned int delimlen)
{
  ...
  *w = 0 ;
  for (*r = 0 ; *r < len ; (*r)++)
  {
    unsigned char c = class[(unsigned char)s[*r]] - '0' ;
    unsigned char action = actions[state][c] ;
    state = states[state][c] ;
    if (action & PUSH0) d[(*w)++] = 0 ;
    if (action & PUSH) d[(*w)++] = s[*r] ;
    if (action & PUSHSPEC) d[(*w)++] = 7 + byte_chr("abtnvfr", 7, s[*r]) ;
    if (action & STORE) store = fmtscan_num(s[*r], 16) << 4 ;
    if (action & CALC) d[(*w)++] = store | fmtscan_num(s[*r], 16) ;
