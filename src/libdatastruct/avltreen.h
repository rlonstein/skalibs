...
extern void avltreen_init (avltreen_ref, avlnode_ref, unsigned char *, unsigned int, uintcmpfunc_t_ref, void *) ;
...
#define AVLTREEB_TYPE(size) struct { avlnode storage[size] ; unsigned char bits[bitarray_div8(size)] ; avltreen info ; }
#define avltreeb_init(t, size, f, p) avltreen_init(&(t)->info, (t)->storage, (t)->bits, size, f, p)
